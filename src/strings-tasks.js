...
function formatTime(minutes, seconds) {
  const minutes1 = String(minutes);
  const seconds1 = String(seconds);
  return `${minutes1.length > 1 ? minutes1 : minutes1.padStart(2, '0')}:${
    seconds1.length > 1 ? seconds1 : seconds1.padStart(2, '0')
  }`;
}
...
function isPalindrome(str) {
  const str2 = str
    .toLowerCase()
    .split('')
    .filter((el) => el.charCodeAt() > 96 && el.charCodeAt() < 122)
    .join('');
  return str2 === str2.split('').reverse().join('').toLowerCase();
}
